...
Makes the top strip the single command surface, while the rest of the UI stays quiet until a route is chosen.

## Structural Difference

Template: command. This proposal changes the first-screen composition and workflow emphasis rather than only changing palette. Its default state is sites, so the first decision is intentionally different from neighboring V9 concepts.
