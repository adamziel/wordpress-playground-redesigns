...
bash v5/scripts/rebuild_index.sh >/dev/null 2>&1 || true
git add v5/designs.json || true

git commit -m "$subj" >/dev/null

attempt=0
while [ $attempt -lt 5 ]; do
  if git push origin main 2>/tmp/v5-push-err; then
    echo "Pushed: $subj"
    exit 0
  fi
  if grep -qE "non-fast-forward|rejected" /tmp/v5-push-err; then
    git pull --rebase origin main >/dev/null 2>&1 || git rebase --abort 2>/dev/null || true
    attempt=$((attempt + 1))
    sleep 1
    continue
  fi
  cat /tmp/v5-push-err >&2
  exit 1
done
