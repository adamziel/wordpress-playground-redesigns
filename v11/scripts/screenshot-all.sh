...
if ! curl -s -o /dev/null -w "%{http_code}" "http://localhost:$PORT/v11/designs/" | grep -q 200; then
  echo "ERROR: local server not reachable on http://localhost:$PORT" >&2
  echo "Start one with:  python3 -m http.server $PORT  in the project root" >&2
  exit 1
fi
...
for slug in $(ls "$DESIGNS_DIR" | sort); do
  ...
  url="http://localhost:$PORT/v11/designs/$slug/"
  echo "→ $slug"
  "${CHROMIUM[@]}" --window-size=1440,1000 --screenshot="$OUT/$slug-desktop.png" "$url" 2>/dev/null || echo "  desktop failed"
  "${CHROMIUM[@]}" --window-size=1440,2200 --screenshot="$OUT/$slug-desktop-tall.png" "$url" 2>/dev/null || echo "  tall failed"
  "${CHROMIUM[@]}" --window-size=390,1200 --screenshot="$OUT/$slug-mobile.png"  "$url" 2>/dev/null || echo "  mobile failed"
done

if curl -s -o /dev/null -w "%{http_code}" "http://localhost:$PORT/v11/" | grep -q 200; then
  "${CHROMIUM[@]}" --window-size=1440,2400 --screenshot="$OUT/_gallery.png" "http://localhost:$PORT/v11/" 2>/dev/null || true
fi
