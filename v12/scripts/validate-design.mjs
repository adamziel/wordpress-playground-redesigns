...
if (!slug || !/^\d{3}-[a-z0-9-]+$/.test(slug)) {
  console.error('Usage: node v12/scripts/validate-design.mjs 001-example');
  process.exit(1);
}
...
async function read(file) {
  try { return await fs.readFile(path.join(dir, file), 'utf8'); }
...
let meta;
try { meta = JSON.parse(await read('meta.json')); }
catch (error) { fail(`invalid meta.json: ${error.message}`); }
...
if (!meta.listPattern || String(meta.listPattern).trim().length < 12) fail('meta.listPattern required (this is the v12 round\'s key field — name the pattern)');
...
if (html.length < 20000) fail(`index.html is too small (${html.length} bytes; need ≥ 20 000)`);
if (readme.length < 700) fail('README.md is too short');

// ---- v11/v12 TASTE CONSTRAINTS ----
...
const required = [
  ...
  ['saved playgrounds', /saved playground|your playgrounds|saved sites/i],
