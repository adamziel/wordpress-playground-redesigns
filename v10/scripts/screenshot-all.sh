...
if ! curl -s -o /dev/null -w "%{http_code}" "http://localhost:$PORT/v10/designs/" | grep -q 200; then
...
for slug in $(ls "$DESIGNS_DIR" | sort); do
  ...
  url="http://localhost:$PORT/v10/designs/$slug/"
  echo "→ $slug"
...
# Also snap the gallery once it exists.
if curl -s -o /dev/null -w "%{http_code}" "http://localhost:$PORT/v10/" | grep -q 200; then
  "${CHROMIUM[@]}" --window-size=1440,2400 --screenshot="$OUT/_gallery.png" "http://localhost:$PORT/v10/" 2>/dev/null || true
fi
